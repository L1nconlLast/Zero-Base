...
const REPORT_PATH = path.join(ARTIFACTS_DIR, 'study-home-contract-smoke-report.json');
...
const main = async () => {
  ...
  try {
    ...
    await waitFor(
      ...
      { timeoutMs: 30000, label: 'card oficial pronto para clique' },
    );
    await screenshot(browser.session, 'study-home-contract-card.png');
    recordStep('study_now_card_real', 'passed', {
      screenshot: 'qa-artifacts/study-home-contract-card.png',
    });

    const pageButtonTexts = await getButtonTexts(browser.session);
    const cardButtonTexts = await getStudyNowCardButtons(browser.session);
    await closeKnownOverlays(browser.session);
    await clickStudyNowCardCta(browser.session);
    await delay(1200);
    const postClickExcerpt = await getBodyTextExcerpt(browser.session);
    const sessionPageDiagnostics = await getSessionPageDiagnostics(browser.session);
    report.postClickExcerpt = postClickExcerpt;
    ...
    await screenshot(browser.session, 'study-home-contract-session.png');
    recordStep('study_now_cta_opens_real_session', 'passed', {
      screenshot: 'qa-artifacts/study-home-contract-session.png',
      pageButtonTexts,
      cardButtonTexts,
      postClickExcerpt,
    ...
    await screenshot(browser.session, 'study-home-contract-session-complete.png');
    recordStep('official_session_answers_recorded', 'passed', {
      answeredOptions,
      screenshot: 'qa-artifacts/study-home-contract-session-complete.png',
    });
    ...
    await screenshot(browser.session, 'study-home-contract-result.png');
    recordStep('official_session_result_persisted', 'passed', {
      screenshot: 'qa-artifacts/study-home-contract-result.png',
    });
    ...
    await screenshot(browser.session, 'study-home-contract-home-after-finish.png');
    recordStep('home_reflects_official_completion', 'passed', {
      screenshot: 'qa-artifacts/study-home-contract-home-after-finish.png',
    ...
    const completedScheduleEntry = scheduleEntriesAfterFinish.find((entry) => entry.id === 'qa-study-home-entry') || null;
    ...
    await waitFor(
      ...
      { timeoutMs: 30000, label: 'home oficial persistida apos reload' },
    );
    ...
    const persistedScheduleEntry = scheduleEntriesAfterReload.find((entry) => entry.id === 'qa-study-home-entry') || null;
    ...
    recordStep('reload_keeps_official_completion_state', 'passed', {
    ...
    report.failureScreenshot = 'qa-artifacts/study-home-contract-failure.png';
    if (browser) {
      await screenshot(browser.session, 'study-home-contract-failure.png').catch(() => undefined);
    }
    await fs.writeFile(REPORT_PATH, JSON.stringify(report, null, 2));
    throw error;
  } finally {
