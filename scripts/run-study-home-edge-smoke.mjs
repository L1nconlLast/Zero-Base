...
const REPORT_PATH = path.join(ARTIFACTS_DIR, 'study-home-edge-smoke-report.json');
const PORT = Number(process.env.STUDY_HOME_EDGE_SMOKE_PORT || 3211);
const BASE_URL = `http://127.0.0.1:${PORT}`;
const API_BASE_URL = (process.env.STUDY_HOME_API_BASE_URL || 'https://zero-base-three.vercel.app').replace(/\/+$/, '');
const EDGE_FLAGS_STORAGE_KEY = 'zb_study_loop_edge_flags';
const apiTraffic = [];
...
const createSeedScript = ({
  ...
  const smokeSeedKey = `study-home-edge-seeded_${normalizedEmail}`;

  return `
    (() => {
      try {
        if (!window.__studyHomeEdgeSmokeInstalled) {
          window.__studyHomeEdgeSmokeInstalled = true;
          window.__studyHomeEdgeSmoke = {
            runtimeErrors: [],
            rejectedPromises: [],
            consoleErrors: [],
          };
          ...
          window.addEventListener('error', (event) => {
            window.__studyHomeEdgeSmoke.runtimeErrors.push(String(event.message || 'runtime-error'));
          });
          ...
          window.addEventListener('unhandledrejection', (event) => {
            ...
            window.__studyHomeEdgeSmoke.rejectedPromises.push(reason);
          });

          const originalConsoleError = console.error.bind(console);
          console.error = (...args) => {
            try {
              window.__studyHomeEdgeSmoke.consoleErrors.push(
                args.map((entry) => {
        ...
        if (window.localStorage.getItem(${JSON.stringify(EDGE_FLAGS_STORAGE_KEY)}) === null) {
          window.localStorage.setItem(
            ${JSON.stringify(EDGE_FLAGS_STORAGE_KEY)},
            ${JSON.stringify(JSON.stringify(edgeFlags))}
          );
        }
      } catch (error) {
        console.error('study-home-edge-smoke-seed-failed', error);
      }
    })();
  `;
};
...
const setStudyLoopEdgeFlags = async (session, flags) =>
  evalInPage(
    session,
    `(() => {
      const nextFlags = ${JSON.stringify(flags)};
      if (!nextFlags || Object.keys(nextFlags).length === 0) {
        window.localStorage.removeItem(${JSON.stringify(EDGE_FLAGS_STORAGE_KEY)});
        return null;
      }
...
const getStudyLoopEdgeSmokeDiagnostics = async (session) =>
  evalInPage(
    session,
    `(() => window.__studyHomeEdgeSmoke || { runtimeErrors: [], rejectedPromises: [], consoleErrors: [] })()`,
  );

const assertNoCriticalBrowserErrors = async (session, label) => {
  const diagnostics = await getStudyLoopEdgeSmokeDiagnostics(session);
  const runtimeErrors = Array.isArray(diagnostics.runtimeErrors) ? diagnostics.runtimeErrors : [];
  const rejectedPromises = Array.isArray(diagnostics.rejectedPromises) ? diagnostics.rejectedPromises : [];
  const consoleErrors = Array.isArray(diagnostics.consoleErrors) ? diagnostics.consoleErrors : [];

  if (runtimeErrors.length > 0 || rejectedPromises.length > 0) {
    throw new Error(`Erros criticos no browser durante ${label}: ${JSON.stringify(diagnostics)}`);
  }
...
const closeKnownOverlays = async (session) => {
  await evalInPage(
    ...
    `(() => {
      ...
      const phaseOverlay = Array.from(document.querySelectorAll('button'))
        .find((button) => normalize(button.textContent || '') === 'fechar'
          && normalize(button.closest('div, section, aside')?.textContent || '').includes('modo interno simulacao de fase'))
        ?.closest('div, section, aside');
      if (phaseOverlay) {
        clickButtonWithin(phaseOverlay, 'fechar');
      }

      const notificationOverlay = Array.from(document.querySelectorAll('[role="alertdialog"]'))
        .find((element) => normalize(element.textContent || '').includes('ativar lembretes de estudo'))
        || Array.from(document.querySelectorAll('button'))
          .find((button) => normalize(button.textContent || '') === 'agora nao'
            && normalize(button.closest('div, section, aside')?.textContent || '').includes('ativar lembretes de estudo'))
          ?.closest('div, section, aside');
...
const clickStudyNowCardButton = async (session, expectedLabel) => {
  const clicked = await evalInPage(
    session,
    `(() => {
      ${BROWSER_NORMALIZE}
      const targetLabel = normalize(${JSON.stringify(expectedLabel)});
      const sections = Array.from(document.querySelectorAll('section'));
      const card = sections.find((section) => normalize(section.textContent || '').includes('para estudar agora'));
      if (!card) return false;
      const button = Array.from(card.querySelectorAll('button')).find((candidate) =>
        normalize(candidate.textContent || '') === targetLabel
      );
  ...
  if (!clicked) {
    const labels = await getStudyNowCardButtons(session);
    throw new Error(`CTA alternativo do card nao encontrado (${expectedLabel}). Botoes: ${labels.join(' | ')}`);
  }
};
...
const main = async () => {
  ...
  try {
    ...
    browser = await launchChrome(10760 + Math.floor(Math.random() * 100));
    await browser.session.send('Page.addScriptToEvaluateOnNewDocument', {
      source: createSeedScript({
        ...
        scheduleEntries: seededScheduleEntries,
        edgeFlags: {
          forceRecommendationEmpty: true,
        },
      }),
    });

    await browser.session.send('Page.navigate', { url: BASE_URL });
    await waitFor(browser.session, 'document.readyState === "complete"', { label: 'load complete' });
    await waitForAnyText(browser.session, ['Inicio', 'Seu plano do dia', 'Bora continuar', 'Vamos dar direcao'], { timeoutMs: 30000 });
    await closeKnownOverlays(browser.session);
    await waitForText(browser.session, 'Para estudar agora', { timeoutMs: 30000 });
    await waitForText(browser.session, 'Ainda nao existe uma recomendacao pronta', { timeoutMs: 30000 });
    await waitForAnyText(browser.session, ['Abrir bloco de questoes', 'Abrir cronograma'], {
      timeoutMs: 30000,
    });
    await screenshot(browser.session, 'study-home-edge-empty.png');
    recordStep('empty_state_renders_fallback', 'passed', {
      screenshot: 'qa-artifacts/study-home-edge-empty.png',
      diagnostics: await assertNoCriticalBrowserErrors(browser.session, 'empty state'),
    });

    const emptyFallbackCtaLabel = await evalInPage(browser.session, `(() => {
      ${BROWSER_NORMALIZE}
      const section = Array.from(document.querySelectorAll('section')).find((candidate) =>
        normalize(candidate.textContent || '').includes('para estudar agora')
      );
      if (!section) return null;
      const button = Array.from(section.querySelectorAll('button')).find((candidate) => {
        const label = normalize(candidate.textContent || '');
        return label === 'abrir bloco de questoes' || label === 'abrir cronograma';
      });
      return button ? button.textContent.trim() : null;
    })()`);
    if (!emptyFallbackCtaLabel) {
      throw new Error('CTA alternativo da home oficial nao foi encontrado no estado empty.');
    }

    await clickStudyNowCardButton(browser.session, emptyFallbackCtaLabel);
    await delay(600);
    await closeKnownOverlays(browser.session);
    await delay(250);
    await closeKnownOverlays(browser.session);
    recordStep('empty_state_cta_click_observed', 'passed', {
      ctaLabel: emptyFallbackCtaLabel,
      locationHref: await evalInPage(browser.session, 'window.location.href'),
      documentTitle: await evalInPage(browser.session, 'document.title'),
      immediateBodyExcerpt: await getBodyTextExcerpt(browser.session),
      diagnostics: await getStudyLoopEdgeSmokeDiagnostics(browser.session),
    });
    await waitFor(
      browser.session,
      `(() => {
        ${BROWSER_NORMALIZE}
        const body = normalize(document.body?.innerText || '');
        return body.includes('organize sua semana')
          || body.includes('seu cronograma da semana')
          || body.includes('depois do foco')
          || body.includes('valide o que voce acabou de estudar')
          || body.includes('validar com questoes')
          || body.includes('pratica recomendada')
          || body.includes('comecar pratica recomendada')
          || body.includes('ativar lembretes de estudo')
          || body.includes('modo interno simulacao de fase');
      })()`,
      { timeoutMs: 30000, label: 'fallback alternativo funcional' },
    );
    await screenshot(browser.session, 'study-home-edge-empty-cronograma.png');
    recordStep('empty_state_cta_is_functional', 'passed', {
      screenshot: 'qa-artifacts/study-home-edge-empty-cronograma.png',
      ctaLabel: emptyFallbackCtaLabel,
      bodyExcerpt: await getBodyTextExcerpt(browser.session),
    });
    await closeKnownOverlays(browser.session);
    ...
    await setStudyLoopEdgeFlags(browser.session, {
      forceHomeError: true,
      forceRecommendationError: true,
    });
    await browser.session.send('Page.reload');
    await waitFor(browser.session, 'document.readyState === "complete"', { label: 'error reload complete' });
    await closeKnownOverlays(browser.session);
    await waitForText(browser.session, 'Nao foi possivel abrir seu proximo estudo', { timeoutMs: 30000 });
    await waitForText(browser.session, 'Tentar novamente', { timeoutMs: 30000 });
    await screenshot(browser.session, 'study-home-edge-error.png');
    recordStep('error_state_renders_retry', 'passed', {
      screenshot: 'qa-artifacts/study-home-edge-error.png',
      diagnostics: await assertNoCriticalBrowserErrors(browser.session, 'error state'),
    });

    await setStudyLoopEdgeFlags(browser.session, {});
    await clickByText(browser.session, 'Tentar novamente');
    await waitFor(
      browser.session,
      `(() => {
        ${BROWSER_NORMALIZE}
        ...
        return sections.some((section) => {
          const buttons = Array.from(section.querySelectorAll('button'))
            .map((button) => normalize(button.textContent || ''))
            .filter(Boolean);
          return buttons.some((label) => label === 'abrir cronograma' || label === 'abrir bloco de questoes')
            && buttons.some((label) => label.includes('estudar agora') || label.includes('continuar agora'));
        });
      })()`,
      { timeoutMs: 30000, label: 'home recuperada apos retry' },
    );
    await screenshot(browser.session, 'study-home-edge-retry.png');
    recordStep('error_retry_recovers_home', 'passed', {
      screenshot: 'qa-artifacts/study-home-edge-retry.png',
      diagnostics: await assertNoCriticalBrowserErrors(browser.session, 'retry recovery'),
    });
    ...
    await closeKnownOverlays(browser.session);
    await clickStudyNowCardCta(browser.session);
    await waitForText(browser.session, 'Falha forcada ao iniciar a sessao oficial.', { timeoutMs: 30000 });
    const stillShowsHomeCard = (await getStudyNowCardDiagnostics(browser.session)).length > 0;
    const enteredStudySession =
      await textExists(browser.session, 'Questao 1 de')
      || await textExists(browser.session, 'Questao 2 de')
      || await textExists(browser.session, 'Sessao pronta para finalizar');
    if (!stillShowsHomeCard || enteredStudySession) {
      throw new Error('Erro ao iniciar sessao oficial corrompeu a home ou abriu sessao quebrada.');
    }
    await screenshot(browser.session, 'study-home-edge-start-error.png');
    recordStep('start_error_keeps_home_stable', 'passed', {
      screenshot: 'qa-artifacts/study-home-edge-start-error.png',
      diagnostics: await assertNoCriticalBrowserErrors(browser.session, 'start error'),
      bodyExcerpt: await getBodyTextExcerpt(browser.session),
    });
    ...
    report.failureScreenshot = 'qa-artifacts/study-home-edge-failure.png';
    if (browser) {
      await screenshot(browser.session, 'study-home-edge-failure.png').catch(() => undefined);
    }
    await fs.writeFile(REPORT_PATH, JSON.stringify(report, null, 2));
    throw error;
  } finally {
