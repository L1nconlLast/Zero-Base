-- ============================================================
-- Migration: XP engine server-side
-- RPCs para cálculo seguro de XP e validação anti-cheat
-- ============================================================

-- Função para calcular XP a partir de uma sessão de estudo (server-side)
-- Aplica as mesmas regras do frontend: minutes * 10 XP
-- Mas adiciona validações anti-cheat:
--   - Máx 480 min (8h) por sessão
--   - Máx 2880 min (48h) por dia
...
DECLARE
  ...
  v_new_level integer;
BEGIN
  -- Validação: minutos positivos e razoáveis
  ...
  -- Validação: limite diário (48h = 2880 min) para evitar abuso
...
-- Função para verificar integridade: recalcula XP total vs sessions
...
BEGIN
  ...
  -- Se inválido, corrige automaticamente
